...
/// Lock-guarded `@unchecked Sendable` spy for `HealthAuthorizationRequesting`.
///
/// Records the number of times `requestAuthorization()` is called.
/// Configure `thrownError` before the call to simulate a failure path.
final class SpyHealthAuthorizer: HealthAuthorizationRequesting, @unchecked Sendable {
