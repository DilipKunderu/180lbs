...
- Composition root: `RootModel` passes the authorizer through to its `OnboardingCoordinatorModel`; `App.swift` picks `HealthKitService()` normally or a `#if DEBUG` `NoOpHealthAuthorizer` under `-ActSkipHealthKitAuthorization`.
  - **Superseded during implementation (architect drift-check ruling, 2026-06-12):** no `NoOpHealthAuthorizer` type was introduced — `nil` is the no-op path (test-pinned on the coordinator); `App.swift` passes `nil` under the flag.
