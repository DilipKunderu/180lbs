...
changelog_vs_previous: |
  - Flow 4 (onboarding bootstrap) corrected to the shipped deferred-atomic-bootstrap design: quit_date/why_sentence/triggers are captured into an in-memory ProfileDraft when leaving OnbQuit; persistence is a single atomic transaction at flow completion after OnbGrocery inserting the PROFILE singleton plus the day-0 WITHDRAWAL_STATE row together (rollback if either fails); no local notifications are registered during onboarding (registration deferred to a future notification-scheduling sub-task). The prior Flow 4 diagram showing a quit-step UPDATE/INSERT and the 7+7+1+weekly notification batch at the quit step was inaccurate.
  - Hard floor text updated from iOS 18.1+ to the shipped iOS 26.0+ target (18.1 remains the absolute Apple Intelligence minimum; 26.0 is the shipped deployment target per the always-latest-iOS directive). Applied in Hard lines, the Deployment diagram, and Integrations.
  - HealthKit read set in Integrations reconciled to the shipped HealthKitReadType: body mass, steps, sleep analysis, resting HR, HRV, VO2 max, dietary water. "dietary energy" and "walking/lifting workouts" moved off the READ side (they remain WRITE-only); steps and sleep analysis added.
  - All other v3 content preserved verbatim.
---

# Act. product design — v4
...
sequenceDiagram
  ...
  App->>Local: bootstrapProfile(Draft) — one GRDB txn: INSERT PROFILE + day-0 WITHDRAWAL_STATE (rollback if either fails)
