---
version: 5
supersedes: design.v4.md
created_at: 2026-06-13
created_by: architect
changelog_vs_previous: |
  - Reconciled the WITHDRAWAL_STATE day-0 contradiction surfaced by a code-vs-design audit. v4 simultaneously asserted (in Flow 4, the v4 changelog, and the onboarding-interface sketch §4) that a day-0 WITHDRAWAL_STATE row is inserted atomically with the PROFILE singleton at onboarding completion, AND (in the Data model "Constraints worth restating" bullet) that "WITHDRAWAL_STATE rows exist only for days where (today - quit_date) ∈ [1, 7]" — which excludes day 0. The shipped `bootstrapProfile` writes the day-0 row, so the code is correct and the constraint bullet was inaccurate. v5 makes the model explicit and consistent in all four places it appears: a single day-0 bootstrap sentinel row (withdrawal_day = 0) is created atomically with PROFILE; days 1–7 are the withdrawal-overlay active window written by the running cessation subsystem; the [1,7] range now describes the CmdWithdrawal surfacing window, not the full set of rows that may exist. The allowed `withdrawal_day` domain is stated precisely (0 for the bootstrap sentinel, 1–7 for the overlay days). The day-0 sentinel carries the "Day 1" hero-word framing, matching the shipped code. Changed: Data model intro prose, the WITHDRAWAL_STATE erDiagram field comments, the "Constraints worth restating" bullet, the Flow 4 prose, and the Cessation stateful-surface prose + diagram.
  - Pinned the previously-unspecified `PROFILE.adherence_pct_cached` formula. v4 described it only as "cached; 7-day rolling" and referenced a >90% "daily check streak (weigh + lift on lift days + meal + EOD)" in the Vision, but never defined the computation, so it was not implementable. v5 adds a "Cached projection — adherence_pct_cached" subsection to the Data model that defines the trailing window, the four expected daily adherence events and how each is detected from the shipped tables, the variable denominator (lift contributes only on Mon/Wed/Fri), the rounding + empty-window rule, the partial-current-day rule, and the recompute-on-write contract. The engineering pair implements `recomputeAdherencePct(db:)` to this spec.
  - Added a one-line note in the Data model that the JSON-valued fields (PROFILE.triggers, URGE_LOG.triggers, GROCERY_LIST.items) are the canonical field names and any storage-layer column naming (e.g. a `_json` suffix) must map back to these names.
  - All other v4 content preserved verbatim.
---

# Act. product design — v5
...
- **Native iOS, SwiftUI on iOS 26.0+ with A17 Pro / M-series chip (hard floor; raised from the 18.1 minimum per the standing always-target-latest-iOS directive — 18.1 remains the absolute Apple Intelligence floor, 26.0 is the shipped deployment target).** No PWA, no web stack, no web-push, no service worker, no Capacitor / React Native / Flutter. SwiftUI is the only UI framework. The hardware floor is a HARD requirement because Apple Intelligence (`FoundationModels.LanguageModelSession`) is the only AI source in the app and has no remote fallback. TestFlight enforces the minimum-OS and minimum-device check; on incompatible hardware the install is refused.
...
erDiagram
  ...
  PROFILE ||--o{ WITHDRAWAL_STATE : "bootstrap day-0 + D1-7 overlay"
  PROFILE {
    ...
    decimal adherence_pct_cached "cached; 7-day rolling; see Cached projection subsection"
  }
  WEIGHT_LOG {
    uuid id PK
  ...
  WITHDRAWAL_STATE {
    uuid id PK
    int withdrawal_day "0 = day-0 bootstrap sentinel (quit day); 1-7 = overlay days"
    date as_of_date "= quit_date for the day-0 sentinel"
    string current_hero_word "Day 1 (day-0 sentinel and D1)|Heavy|Today|Lighter|Sleep|Energy|Through"
    boolean is_worst_day "true on day 3 only; false on the day-0 sentinel"
  }
```
...
- `WITHDRAWAL_STATE.withdrawal_day` has a precise allowed domain: `0` for the single day-0 bootstrap sentinel row, and `1`–`7` for the withdrawal-overlay days. A single day-0 sentinel row (`withdrawal_day = 0`, `as_of_date = quit_date`) is created atomically with the PROFILE singleton at onboarding completion via `bootstrapProfile` (see Flow 4) — it represents the quit day itself, so the cessation pillar is never left without a withdrawal_state row on first launch. Rows for days `1`–`7` are written by the running app (the cessation subsystem) on those days. The `(today - profile.quit_date) ∈ [1, 7]` range describes the *active-overlay / surfacing window* — when `CmdWithdrawal` surfaces on the Today coordinator and the day-specific hero words apply — NOT the full set of `WITHDRAWAL_STATE` rows that may exist (the day-0 sentinel is outside that surfacing window but is still a valid, intentionally-written row). After day 7 no further withdrawal_state rows are written and `CmdWithdrawal` retires from the Today coordinator's repertoire (see "Stateful surfaces" → Cessation clean / relapse streak, and → Today coordinator).
...
### Cached projection — `adherence_pct_cached`

`PROFILE.adherence_pct_cached` is the >90% "daily check streak" the Vision references: the share of expected daily adherence events that were actually answered over a trailing 7-day window. It is a cached projection over the local SQLite tables (CloudKit + local mirror both carry the column), recomputed by `recomputeAdherencePct(db:)` on every write to any of the underlying tables (`WEIGHT_LOG`, `LIFT_SESSION`, `MEAL_LOG`, `DEVIATION_LOG`, `SMOKE_CHECK`) before that write is saved — exactly as the other cached columns are. The definition below is the single source of truth for the implementation.

**Window.** The trailing 7 calendar days *ending yesterday*, evaluated in device-local wall-clock (`TimeZone.current`). That is, the window is the 7 fully-elapsed days `[today − 7, today − 1]` inclusive. The current (partial) day is excluded entirely from both numerator and denominator. This is the simplest implementable rule that guarantees a not-yet-due event today can never count as a miss — the current day only enters the window once it has fully elapsed (it becomes "yesterday" tomorrow). Each day in the window is evaluated against its own weekday (so lift days are determined per-day, not for today).

**Expected daily adherence events** (aligned to the Vision's "weigh + lift on lift days + meal + EOD"), and how each is detected from the shipped tables for a given calendar day `D`:

- **weigh-in** — counts as answered iff a `WEIGHT_LOG` row exists whose local-calendar day equals `D`. Expected every day in the window.
- **lift** — expected ONLY on lift days (`D` is a Monday, Wednesday, or Friday). On a lift day, counts as answered iff a `LIFT_SESSION` with `completed = true` exists for `D`. On non-lift days lift is NOT an expected event (it contributes nothing to the denominator for `D`).
- **meal** — counts as answered iff a `MEAL_LOG` row OR a `DEVIATION_LOG` row exists for `D`. A logged deviation counts as "answered": per the `MEAL_LOG` ↔ `DEVIATION_LOG` mutual-exclusion model, exactly one of the two may exist for a date, and the user having *answered* the meal question (with either a logged meal or an honestly logged deviation) is what adherence measures. (This is a deliberate choice: adherence tracks "did the user respond to the day's prompts", not "did the user stay on plan". Off-plan honesty is not a miss.) Expected every day in the window.
- **EOD** — counts as answered iff a `SMOKE_CHECK` row exists for `D`, regardless of `answer` (`clean` or `relapse`). The forced choice having been completed is what counts; a logged relapse is still an answered EOD. Expected every day in the window.

**Denominator.** The total number of *expected* events across the 7-day window. weigh-in, meal, and EOD each contribute one expected event per day (3 × 7 = 21). lift contributes one expected event per *lift day* that falls in the window. Because the window is a fixed 7 consecutive calendar days, it always contains exactly 3 lift days (one each of Mon/Wed/Fri) — so the denominator is normally `21 + 3 = 24`. The denominator is written to vary by lift-day count rather than hard-coded, because the rule must remain correct if the window length or lift-day schedule is ever changed by a future design event; the implementation MUST compute the lift-day count from the window's actual weekdays, not assume 3.

**Numerator.** The count of expected events in the window that were answered, by the detection rules above.

**Formula and edge cases.**

- `adherence_pct_cached = round(numerator / denominator × 100)`, using round-half-up to the nearest integer (the column is `decimal` but always holds a whole-number percentage 0–100).
- If the denominator is 0 (the empty-window / division-by-zero case — e.g. a brand-new install where the trailing 7 fully-elapsed days predate `quit_date` and contain no expected events because the profile did not yet exist), `adherence_pct_cached = 0`. The implementation treats "no expected events yet" as 0% rather than 100% so a fresh install does not display a misleading perfect streak.

The partial-current-day rule above (window ends yesterday) is the operative choice: today's not-yet-due events never count as misses because today is not in the window at all.

## Key flows

The flows below are the load-bearing interactions. Every other Cmd* screen follows a variant of Flow 1 (write something, mirror to HealthKit, save to CloudKit) or Flow 3 (local notification opens the right screen). Flow 4 (onboarding bootstrap) is included because the quit-date capture is a one-shot bootstrap event that the entire cessation pillar depends on; in the shipped milestone that capture is deferred into an in-memory `ProfileDraft` and persisted atomically only at flow completion after `OnbGrocery` (a single GRDB transaction inserting the singleton PROFILE row plus the day-0 WITHDRAWAL_STATE bootstrap sentinel row — `withdrawal_day = 0`, `as_of_date = quit_date` — together via `bootstrapProfile`), and no local notifications are registered during onboarding (registration is deferred to a future notification-scheduling subsystem). Flow 5 is new in v3: the Sunday 20:00 weekly insight, generated on-device via Apple Intelligence and mirrored across devices via CloudKit.
...
### Flow 4 — bootstrap: onboarding captures immutable quit_date into a draft, persists atomically at completion

```mermaid
sequenceDiagram
  actor User
  participant App as SwiftUI app
  participant Draft as ProfileDraft (in-memory)
  participant Local as Local SQLite (GRDB)
  participant CK as CloudKit private DB
  User->>App: completes OnbQuit (trigger chips + why sentence)
  User->>App: tap "I am a non-smoker."
  App->>Draft: set quit_date=today, why_sentence=..., triggers=... (no DB write)
  App-->>User: advance to OnbRotation
  Note over User,App: OnbRotation … OnbGrocery (further draft answers accumulate)
  User->>App: complete OnbGrocery
  App->>Local: bootstrapProfile(Draft) — one GRDB txn: INSERT PROFILE + day-0 WITHDRAWAL_STATE sentinel (withdrawal_day=0, as_of_date=quit_date) (rollback if either fails)
  App->>CK: propagate PROFILE + WITHDRAWAL_STATE as CloudDelta.save (async)
  CK-->>App: ack (or queues if offline)
  App-->>User: route into the running app (Today coordinator)
  Note over App: local-notification registration is DEFERRED (future notification-scheduling subsystem)
```

The bootstrap is a single atomic GRDB transaction: it inserts the PROFILE singleton AND the day-0 `WITHDRAWAL_STATE` bootstrap sentinel together (`withdrawal_day = 0`, `as_of_date = quit_date`, `current_hero_word = "Day 1"`, `is_worst_day = false`), rolling back both if either fails. The day-0 sentinel represents the quit day itself; it carries the "Day 1" hero-word framing (the user's first clean day is framed as Day 1). It is deliberately written outside the `[1, 7]` surfacing window so the cessation pillar always has a withdrawal_state row from first launch — the D1-7 overlay rows are written later by the running cessation subsystem on those days (see "Stateful surfaces" → Cessation clean / relapse streak). The shipped `bootstrapProfile` does exactly this.
...
Owned by the SwiftUI app's `CessationCoordinator` with the local SQLite mirror as the read path for the Live Activity and the CloudKit private database as the cross-device sync path. At onboarding completion a single day-0 `WITHDRAWAL_STATE` bootstrap sentinel row (`withdrawal_day = 0`, `as_of_date = quit_date`) is written atomically with PROFILE (see Flow 4); the day-0 sentinel carries the "Day 1" hero-word framing and `is_worst_day = false`. It is the quit-day row and sits outside the surfacing window. The withdrawal overlay then surfaces `CmdWithdrawal` from the Today coordinator and uses day-specific hero words for the days where `today ∈ [quit_date+1, quit_date+7]`; the cessation subsystem writes the matching `WITHDRAWAL_STATE` rows (`withdrawal_day` 1–7) on those days. Day 3 is the worst-day hero (`is_worst_day = true`). After `today > quit_date+7` no further withdrawal rows are written and the overlay retires.
...
stateDiagram-v2
  ...
  state Clean {
    [*] --> Day0Sentinel
    Day0Sentinel --> Withdrawal_D1to7 : today in [quit_date+1, quit_date+7]
    Day0Sentinel --> Steady : today > quit_date+7
...
The `Day0Sentinel` sub-state corresponds to the `withdrawal_day = 0` row written at bootstrap (the quit day itself, before any D1-7 overlay day has begun). It is a valid clean state outside the `[1,7]` surfacing window; on the next morning the machine moves into `Withdrawal_D1to7` (or, in the degenerate case where the device clock has already advanced past `quit_date+7`, straight to `Steady`).

### Today coordinator (which Cmd* screen the app surfaces right now)

Owned by the SwiftUI app's `TodayCoordinator`. Inputs: wall-clock time, weekday (lift days are Mon/Wed/Fri), today's completed checks from local store, hydration state, cessation state. Output: exactly one Cmd* screen at any moment. `Urge` and `Withdrawal` are modal overlays reachable from any state; the diagram models a single entry edge from `Fasting` for compactness, but in practice the sheet returns to whichever state was current before invocation. The `Withdrawal` overlay is reachable only while `today ∈ [quit_date+1, quit_date+7]` (the surfacing window); the day-0 sentinel does not surface a `CmdWithdrawal` card.
...
flowchart TB
  subgraph dev [User iPhone — bundle id com.act.coach]
    SwiftApp[SwiftUI app target<br/>iOS 26.0+ / A17 Pro / M-series]
...
| Nightly ~23:55 | `BGAppRefreshTask` re-registration | App re-registers tomorrow's local notifications if `wake_time` / `meal_window_*` / `bed_time` changed |
...
|---|---|---|---|
| HealthKit (Apple) | Per-device biometrics, workouts, dietary data | Read: body mass, steps, sleep analysis, resting HR, HRV, VO2 max, dietary water. Write: dietary energy (meal), dietary water (shake + manual), lifting/swim/walk workouts | Per-type authorization denied → graceful fallback to manual entry on each surface; recovery stats show "no data yet — wear your Watch on training days" |
...
| Apple Intelligence `FoundationModels` (Apple) | On-device LLM inference | Write: structured prompts. Read: text completions | Unavailable on older hardware → TestFlight enforces the iOS 26.0+ A17 Pro / M-series floor (18.1 remains the absolute Apple Intelligence minimum) so this shouldn't occur in practice; if it does, `CmdReview` shows the four stat rows without the insight callout. No remote fallback (forbidden by hard line). Thermal throttling or rate limiting → empty response; treated as "no insight this week", retried next `BGProcessingTask` wake |
